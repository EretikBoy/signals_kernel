...
От pickle отказались — слишком хрупкий формат для данных, которые должны жить
годами и пережить смену версий программы. Вместо одного файла проект хранится
папкой:
...
Так получилось решить сразу две старые беды. Во-первых, запись идёт через
временный файл и os.replace — если погаснет свет посреди сохранения, проект
не превратится в кашу из половины старых и половины новых данных. Во-вторых,
автосейв переписывает на диск только те анализы, что реально изменились
(`Analysis.dirty`), а не все разом — если их в проекте тысяча, заметной паузы
в интерфейсе не будет. CSV с данными каналов читаются тем же парсером, что и
обычные файлы — формат совпадает, ничего отдельного писать не пришлось.

Чтобы такой проект было удобно переслать или убрать в архив, есть pack()/
unpack() — упаковка в один .sigproj (это обычный zip, заодно и сжимает).
"""
