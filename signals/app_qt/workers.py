...
def scan_instruments(log) -> list[dict]:
    """Просканировать систему на подключённое оборудование и опознать приборы.

    * VISA-приборы: перечисляем ресурсы, опрашиваем *IDN?, сопоставляем с плагинами
      по их отпечатку (matches_idn). Каждый найденный ресурс и его ответ
      попадают в журнал — пригодится, если понадобится снять отпечаток нового
      прибора, которого ещё нет среди плагинов.
    * не-VISA приборы (Hantek): спрашиваем у плагина discover() (скан USB).
    ...
    try:
        ...
        for res in resources:
            idn = None
            try:
                # *IDN? — это SCPI-команда «спроси и ответь», она есть только у
                # MessageBasedResource (USB/GPIB/serial/TCPIP — все приборы, с
                # которыми мы тут работаем); другие виды ресурсов (например,
                # «голый» USB RAW) сюда не попадут — откроются, но не ответят
                inst = cast(MessageBasedResource, rm.open_resource(res))
                try:
                    inst.timeout = 2500
                except Exception:
                    pass
    ...
    for e in INSTRUMENTS:
        ...
        if not callable(disc):
            continue
        # discover — необязательный метод плагина (объявлен через getattr,
        # потому что не все приборы умеют сканироваться сами); тип берём из
        # фактической сигнатуры в inst_hantek.discover, а не из Any
        disc = cast("Callable[..., list[dict] | None]", disc)
