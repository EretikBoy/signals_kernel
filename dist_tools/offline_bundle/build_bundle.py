...
GET_PIP_URL_LATEST = "https://bootstrap.pypa.io/get-pip.py"
GET_PIP_URL_LEGACY = "https://bootstrap.pypa.io/pip/{major}.{minor}/get-pip.py"
#: get-pip.py из bootstrap.pypa.io больше не ставится на Python < 3.10 —
#: для них нужен версионный бутстрап (см. GET_PIP_URL_LEGACY)
GET_PIP_MIN_VERSION = (3, 10)
...
def _get_pip_url_for(version: str) -> str:
    major, minor, *_ = (int(p) for p in version.split("."))
    if (major, minor) < GET_PIP_MIN_VERSION:
        return GET_PIP_URL_LEGACY.format(major=major, minor=minor)
    return GET_PIP_URL_LATEST
...
def _enable_pip(pyruntime: Path, arch: str) -> Path:
    ...
    _download(_get_pip_url_for(version), get_pip)
